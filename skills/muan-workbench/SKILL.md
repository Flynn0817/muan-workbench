...
### 10.22 「自然变密」下的密图可读性（2026-09-23 九版）

用户选择「自然变密」（不给正文证据设上限）。因此必须保证**库变密时图还能看**——
之前所有布局参数都是按 4 篇调的，我造了一个 40 篇的密集库才发现问题。

#### 造密集测试库的办法
`muan-dense-tools/set_dense.js`：生成 40 篇，4 个理论派别 × 3 个分类标签 × 4 种期刊 × 6 位作者。
**这个库会变成完全图**（780 对全部有连线：强 380 / 明确 400）——正好是最坏情况压力测试。
（注意：脚本不要和沙箱放同一目录，否则沙箱 `rmSync` 会把脚本自己删掉。）

#### 四个必须改的参数（都是「按 4 篇调」留下的坑）
1. **理想距离 `k` 的下限写死 70px**：40 篇时画布放不下。改为分档
   `litGraphScale(n)`：≤14 → 70／15–30 → 46／31–60 → 38／>60 → 32。
   依据是「椭圆可用面积 ÷ 节点数」开方（手机 329×490 时 ≈ sqrt(89000/40) ≈ 47，留余量取 38）。
2. **节点半径基础 7px、度数系数 1.7**：40 篇里度数全是 39，半径全变 16.5px，根本放不下。
   加 `rMax` 分档（11/8/6/5），半径 = `rBase + min(rMax, deg*degK)`。
3. **向心力必须是「单节点」量级，不能逐对累加**——斥力逐对累加（∝n），向心不衰减，
   节点一多平衡半径就被压回中心。按力学平衡推：`R = (0.22·n·k²/cP)^(1/3)`，
   要让 R 稳定就得让 cP ∝ 1/√n。新增 `litGraphPull(n) = max(0.006, 0.022·√(4/n))`。
   （实测：不改这条，40 篇只占画布 51%×34% = 缩成一团。）
4. **连线拉力必须按度数归一化**——这是密图塌陷的**真凶**：连线是「弹簧拉向固定目标距离」，
   完全图里每个节点被 39 条边同时拉，总拉力是斥力的几十倍。
   标准做法是单边强度乘 `1/√(min(degA,degB))`：枢纽节点的总牵引量才有界。
   实测改完占宽从 51% → **86%**，占高 34% → 60%，且居中。

#### 视觉层
- 连线数 >400 / >150 / >60 时整体降透明度（0.30 / 0.48 / 0.72）并收线宽（×0.72）——
  几百条线按单条的不透明度画会糊成一片深色，层次全丢；降透明度后**疏密本身成为读法**。
  悬停的那条永远画满，不被密度压暗。
- 连线 >120 条时在图上给一句可操作提示（指向「明确及以上 / 只看强关联」），**不静默改档位**。

#### 回归验证（改物理必做）
用小库（真实 4 篇）和大库（合成 40 篇）分别验：
- 4 篇：4 节点成菱形、居中偏移 (-1,4)、占 69%×58%、0 连线散点 —— **与改动前一致**（1/√deg 让 deg 1–3 的小库几乎不受影响）
- 40 篇：占 86%×60%、居中；四色聚类肉眼可分；编号可读；`强 380（另 400 条已隐藏）` 档位生效
- 15 导航 + 科研 9 标签 + 文献库 6 视图 + 星图三档：无报错、手机 390 / 桌面 1280 均无横向溢出

#### 一个反复踩的坑
补丁脚本里写 `R(name, \`...find...\`, \`...repl...\`)` 时，**别在模板串里顺手把 R 的收尾 `);` 也写进去**——
本次就在 `litGraphScale` 函数后面多写了一个 `});`，导致 index.html 语法错误，
靠 `vm.Script` 校验 + 「打印出错行上下 5 行」的定位脚本才发现。**每次写完必须立刻跑语法校验。**

#### 补：公开脚本不能含本机信息（2026-09-23 九版修正）
把 sync-repo.js 提交进公开仓库后才发现：它**硬编码了真实项目 ref、姓名、昵称、本机绝对路径**——
等于「泄露扫描器自己泄露」，而且扫描时会不断命中自己。现已改为：

- 脚本里只留**通用规则**（Supabase 密钥前缀、三段式 JWT、service_role + 长凭据、邮箱、长令牌赋值），
  不依赖任何个人标识；
- **绝对路径与个人化规则**都移到同目录 `sync-repo.local.json`（已 gitignore），
  也支持 `MUAN_SRC` / `MUAN_REPO` 环境变量覆盖；
- 扫描时**跳过扫描器自身与规则文件**（它们按定义包含这些模式，不是泄露）；
- 规则改动后仍要反向验证（造含真敏感值的临时文件，确认 8 类都抓到、`xlsx@0.18.5` /
  `you@example.com` / `@media` 不误报）。

**通用教训**：任何要提交到公开仓库的脚本，都要先跑一遍「它自己会不会命中自己的敏感词」。
`git grep -n "<本机路径>\|<姓名>\|<项目 ref>"` 是最后一道闸。
（历史提交里仍留有这些值，单纯删当前文件不能清历史——需要重写历史并 force push 才能彻底移除。）
